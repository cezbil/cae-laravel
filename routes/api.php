...
use App\Http\Controllers\Event\FindFlightsByStartLocationController;
use App\Http\Controllers\Event\FindFlightsForNextWeekController;
use App\Http\Controllers\Event\FindStandByForNextWeekController;
...
Route::middleware('auth:sanctum')->prefix('/events')->group(function () {
    ...
    Route::get('/flights/location', [FindFlightsByStartLocationController::class, '__invoke'])->name('flights.from.location');
    Route::get('/sby', [FindStandByForNextWeekController::class, '__invoke'])->name('sby.nextweek');
});
